root_folder = 'G:\My Drive\Microscope\Calibration\SingleSiteAblationTest\20220504';
analysis_folder = fullfile(root_folder, 'result');
pixel_size_um = [1, 1, 1];
im_name_base = 'WBIM_abl_';
im_list = cell(1, 2);
site_idx = 8;
x0_list = [67, 67, 167, 167, 167, 142, 167, 167];
dist_list = [150, 150, 50, 50, 50, 75, 50, 50];
z_list = [50, 100, 50, 50, 50, 50, 50, 50];
for i = 1 : 2
    im_fn = sprintf('%s%05d.tif', im_name_base, (site_idx-1)*2 + i);
    if i == 1
...
%%
x_list = x0_list(site_idx) : dist_list(site_idx) : 700;
fig_hdl = figure;
...
cbar_1.Label.String = 'Intensity';
colormap(ax_hdl_4, 'jet');
ax_hdl_4.CLim = [0, 2e4];
%%
fig_fp = fullfile(vis_folder, sprintf('%s_ablation_site_im_pm_%dum.png', data_name, disp_dz));
fun_print_image_in_several_formats(fig_hdl, fig_fp);