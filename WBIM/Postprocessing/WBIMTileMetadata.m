...
%         channel_offset (1, 4) int16
    ...
    methods
        function obj = WBIMTileMetadata(exp_group, exp_name, layer_idx, acq_mode, ...
            ...
            obj.add_dir_info();
            if ~overwriteQ
                tile_folder_fp = fullfile(obj.save_root_folder, obj.fprr_tile_folder);
                while isfolder(tile_folder_fp)
    ...
    methods
        ...
        function val = get.h5_file_exist_Q(obj)
            h5_fp = fullfile(obj.save_root_folder, obj.fprr_tile);
            % I am not sure about the performance here. 
            if isfolder(obj.data_manager.SERVER_ROOT_PATH)
                server_h5_fp = strrep(h5_fp, obj.data_manager.fp_acquisition_disk, ...
                    obj.data_manager.fp_acquisition_server);
                val = isfile(server_h5_fp) || isfile(h5_fp);
            else
                val = isfile(h5_fp);
            end
        ...
        %% Checking 
        function file_size_ratio = check_SI_file_size(obj)
            file_info = dir(obj.SI_filepath);
            if ~isempty(file_info)
                target_size = prod(obj.stack_size) * 2; % Assume int16 data type here.
                file_size_ratio = target_size / file_info.bytes;
            else
                file_size_ratio = nan;
            end
        end
    ...
    methods
        ...
        function tile_cell = load_tile(obj, channel_id, hdf5_fp)
            arguments
                obj WBIMTileMetadata
...
%             end
    ...
    methods(Static)
        function output = load(filepath)
            ...
            filepath = DM.fp_download_if_not_exist(filepath);
            if isfile(filepath)
    ...
    methods(Static)
        ...
        function c_info = collect_tiles_info(tile_info)
            ...
            potential_unique_prop = {'grid_size', 'channel', 'stack_size', ...
                'stack_size_um', 'pixel_size_um', 'overlap_size_um', 'overlap_size'};
            ...
            for iter_prop = 1 : num_prop
                ...
                if numel(selected_idx) == 1 && ismember(tmp_prop_name, potential_unique_prop)
