classdef Logger < handle & matlab.mixin.SetGetExactNames & ...
    ...
    properties (AbortSet)
        ...
        % Level of message to send email 
        EmailThreshold (1,1) mlog.Level = mlog.Level.ERROR
    ...
    methods
        ...
        function obj = Logger(name, filePath)
            ...
            if ~isempty(folder) && folder ~= "" && ~isfolder(folder)
                mkdir(folder);
            end
...
%                 obj.LogFile = fullfile(tempdir, fileName);
        ...
        function fopenLogFile(obj, permission)
            ...
            if ismember(obj.FileID, fopen('all'))
                warning('on', 'MATLAB:fopen:FopenAllStagedRemoval');
