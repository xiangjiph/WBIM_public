...
%         HOST_SYSTEM
    ...
    methods
        function obj = WBIMFileManager(host_name)
            ...
            switch host_name
                case 'pia'
                    obj.SCRIPT_PATH = 'C:\Users\dklab\Documents\GitHub\WBIM';
                    obj.SCRATCH_ROOT_PATH = 'H:\WBIM';
                    obj.DATA_ROOT_PATH = 'D:\WBIM';
                    obj.SERVER_ROOT_PATH = 'Z:\WBIM';
%                     if isfolder(obj.SERVER_ROOT_PATH)
%                         % Try to save data directly to the server. 
%                         obj.DATA_ROOT_PATH = obj.SERVER_ROOT_PATH;
%                     else
%                         fprintf("Network drive is not connected. Using local HDD for data storage.\n");
%                     end                    
                    obj.SERVER_SCRIPT_PATH = [];
                    obj.PYTHON_EXE_PATH = 'C:/Users/dklab/miniconda3/Scripts/activate & conda activate WBIM & C:\Users\dklab\miniconda3\envs\WBIM\python.exe';
                    obj.PYTHON_SCRIPT_PATH = 'C:\Users\dklab\Documents\GitHub\SITiffConverter';
            end
        end
    end
    %% Gets
    methods
        ...
        function fp = get.fp_acquisition_server(obj)
            fp = fullfile(obj.SERVER_ROOT_PATH, 'Acquisition');
        end
    ...
    methods(Static)
        ...
        function fn = fn_stitching_volume_info()
            fn = 'volume_info.mat';
        end

        function fn = fn_stitching_df_info()
            fn = 'deformation_field.mat';
        end
    end
    methods        
    ...
    methods
        ...
        function fp = fp_RI_log_file(obj, exp_group, exp_name)
            fp = fullfile(obj.fp_acquisition_disk, obj.fpr_experiment(exp_group, exp_name), ...
                obj.fn_RI_log_file(exp_group, exp_name));
        end
    ...
    methods(Static)
        ...
        function fn = fn_RI_log_file(exp_group, exp_name)
            fn = sprintf('%s_%s_RI_log.txt', exp_group, exp_name);
        end
    ...
    methods(Static)
        ...
        function exit_code = write_tile_h5_file(h5_fp, im_stack, channel_list, complete_overwriteQ, opts)
            arguments
                h5_fp
                im_stack
                channel_list (:, 1) {mustBeNumeric};
                complete_overwriteQ (1, 1) logical = false;
                opts.match_python_h5_Q (1, 1) logical = false;
            end
            if isnumeric(im_stack)
                im_stack = {im_stack};
                if ~isempty(channel_list)
                    assert(isscalar(channel_list));
                end
            end
            raw_im_type = class(im_stack{1});
            num_channel = numel(im_stack);
            if isempty(channel_list)
                channel_list = 1 : num_channel;
            else
                assert(numel(channel_list) == num_channel);
            end    
            h5_info = [];
            if isfile(h5_fp)
                if complete_overwriteQ
                    delete(h5_fp);
                else
                    warning("Target h5 file already exist");
                    h5_info = h5info(h5_fp);
                end
            end
            
            for iter_ch = 1 : num_channel
                tmp_im = im_stack{iter_ch};
                if opts.match_python_h5_Q
                    tmp_im = permute(tmp_im, [2, 1, 3]);
                end
                tmp_im_size = size(tmp_im);
                % Write H5
                tmp_h5_ds = sprintf('/CH%d/raw', channel_list(iter_ch));
        ...
        function tile_cell = load_tile_h5_file(hdf5_fp, channel_id)
            num_channel = numel(channel_id);
            tile_cell = cell(num_channel, 1);
            for i_c = 1 : num_channel
                tmp_ch = channel_id(i_c);
                tile_cell{i_c} = permute(h5read(hdf5_fp, ...
                    sprintf('/CH%d/%s', tmp_ch, 'raw')), [2,1,3]);
            end
        end
    end
    %% Copy files
...
%                         fp = strrep(fp, obj.ROOT_PATH, obj.SERVER_ROOT_PATH);
                    ...
                    case 'ThinkPadP16'
                        ...
                        head_disk = fp(1);
                        s1 = sprintf('%s:', head_disk);
                        s2 = sprintf('/mnt/%s', lower(head_disk));   
                        fp_wsl = strrep(strrep(fp, s1, s2), filesep, '/');
                        [folder_path, ~, ~] = fileparts(fp);
                        if ~isfolder(folder_path)
                            mkdir(folder_path);
                        end
        ...
        function download_tile_info_in_experiment(obj, exp_group_name, exp_name)
            ...
            head_disk = root_data_folder(1);
            s1 = sprintf('%s:', head_disk);
            s2 = sprintf('/mnt/%s', lower(head_disk));
            ubuntu_fp = strrep(strrep(root_data_folder, s1, s2), filesep, '/');
    ...
    methods(Static)
        ...
        function exit_code = copy_file_local_unix(source_fp, target_fp, asynchronous_Q)
            if nargin < 3
                asynchronous_Q = false;
            end
            cmd_str = sprintf('rsync -ra --checksum "%s" "%s"', source_fp, target_fp);
            if asynchronous_Q
                cmd_str = sprintf('%s &', cmd_str);
            end
            exit_code = system(cmd_str);
