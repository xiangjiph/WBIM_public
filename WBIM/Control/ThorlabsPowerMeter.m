classdef ThorlabsPowerMeter < matlab.mixin.Copyable
    ...
    properties (Constant, Hidden)
        ...
        METERPATHDEFAULT = fullfile(fileparts(mfilename('fullpath')),...
            'Thorlabs_DotNet_dll');
    ...
    methods
        function obj = ThorlabsPowerMeter()
            ...
            obj.loaddlls;
            [obj.resourceName, obj.modelName, obj.serialNumber, obj.Manufacturer, ...
                obj.DeviceAvailable] = obj.listdevices;
        ...
        function obj_copy = connect(obj,resource,resource_index,ID_Query,Reset_Device)
            ...
            if ~obj.isConnected
                try
                    ...
                    % modified to account for cell by Sincheng 10/25/23
                    obj_copy.deviceNET = Thorlabs.TLPM_64.Interop.TLPM(resource{resource_index},logical(ID_Query),logical(Reset_Device));
        ...
        function disconnect(obj)
            ...
            if obj.isConnected
                fprintf('\tDisconnecting ... %s\r',obj.resourceNameConnected{1});   %modified to account for cell by Sincheng 10/25/23
                try
        ...
        function [resourceName, modelName, serialNumber, Manufacturer, DeviceAvailable] = listdevices()  % Read a list of resource names
            ...
            if count>0
                for i = 0:1:count-1
                    ...
                    resourceNameArray{i+1,:} = char(descr{1}.ToString);   %modified to cell array Sincheng 10/25/23
                    modelNameArray{i+1} = char(descr{2}.ToString);
                    serialNumberArray{i+1,:} = char(descr{3}.ToString);   %modified to cell array Sincheng 10/25/23
                    ManufacturerArray{i+1,:} = char(descr{4}.ToString);   %modified to cell array Sincheng 10/25/23
        ...
        function loaddlls() % Load DLLs
            ...
            if ~exist(ThorlabsPowerMeter.TLPMCLASSNAME,'class')
                try   % Load in DLLs if not already loaded
                    NET.addAssembly(fullfile(ThorlabsPowerMeter.METERPATHDEFAULT, ThorlabsPowerMeter.TLPMDLL));
